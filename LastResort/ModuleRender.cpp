...
bool ModuleRender::BlitEx(SDL_Texture* texture, int x, int y, SDL_Rect* section, SDL_RendererFlip axis, double angle)
{
	bool ret = true;
	SDL_Rect rect;
	...
	if (SDL_RenderCopyEx(renderer, texture, section, &rect, angle, NULL, axis) != 0)
	{
		LOG("Cannot blit to screen. SDL_RenderCopy error: %s", SDL_GetError());
		ret = false;
	}
